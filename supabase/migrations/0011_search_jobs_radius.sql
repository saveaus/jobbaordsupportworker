-- Let search choose how far from the listed suburb to look.
...
begin
  ...
  v_radius := least(greatest(coalesce(p_radius_km, 50), 1), 500);
  ...
  where j.status = 'live'
    ...
    and (
      ...
      or (
        ...
        and haversine_km(p_lat, p_lng, j.lat, j.lng) <= v_radius
      )
    )
  order by
    case
